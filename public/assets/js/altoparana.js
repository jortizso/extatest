...
function createAnuncio(anuncio) {
    ...
    // Corregir el uso de backticks en lugar de comillas simples
...
function fetchAnuncios() {
    ...
    get(child(dbRef, 'escorts/zona/altoparaná')).then((snapshot) => {
        if (snapshot.exists()) {
            const anuncios = snapshot.val();
            for (let id in anuncios) {
                createAnuncio(anuncios[id]);
            }
        } else {
